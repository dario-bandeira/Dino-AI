...
def load_image(
    ...
    scenery_image = pygame.image.load(fullname)
    scenery_image = scenery_image.convert()
    if colorkey is not None:
        if colorkey == -1:
            colorkey = scenery_image.get_at((0, 0))
        scenery_image.set_colorkey(colorkey, RLEACCEL)

    if sizex != -1 or sizey != -1:
        scenery_image = pygame.transform.scale(scenery_image, (sizex, sizey))

    return scenery_image, scenery_image.get_rect()
...
def load_sprite_sheet(
    ...
    for i in range(0, ny):
        for j in range(0, nx):
            sprite_rect = pygame.Rect((j * sizex, i * sizey, sizex, sizey))
    ...
    while not gameover:
        for user_event in pygame.event.get():
            ...
            if user_event.type == pygame.KEYDOWN:
                ...
                if user_event.key == pygame.K_EQUALS:
                    FPS += 10
                if user_event.key == pygame.K_MINUS:
                    FPS -= 10

            if user_event.type == pygame.KEYUP:
                if user_event.key == pygame.K_DOWN:
        ...
        if len(cacti) < 2:
            if len(cacti) == 0:
                ...
                for l_cact in last_obstacle:
                    if l_cact.rect.right < width * 0.7 and random.randrange(0, 50) == 10:
                        last_obstacle.empty()
                        last_obstacle.add(Cactus(gamespeed, 40, 40))

        if len(pteras) == 0 and random.randrange(0, 200) == 10 and counter > 500:
            for l_pteras in last_obstacle:
                if l_pteras.rect.right < width * 0.8:
                    last_obstacle.empty()
        ...
        if pygame.display.get_surface() is not None:
            ...
            gamefont.render_to(screen, (10, 30), "Do chão: " + str(
                height - obstaculo_mais_proximo.rect[1] - obstaculo_mais_proximo.rect[2]), (0, 0, 0))
            ...
            gamefont.render_to(screen, (10, 120),
                               "Altura Dino: " + str(height - playerdino.rect[1] - playerdino.rect[2]), (0, 0, 0))
