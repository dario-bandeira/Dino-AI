...
class NeuralNetwork:
    def __init__(self):
        self.ai_syn1 = np.random.uniform(-1, 1, (2, 3))
...
def select_bests():
    nn_array.sort(key=lambda x: x.fitness, reverse=True)
    del nn_array[2:]
    ...
    while not gameQuit:
        ...
        while not gameOver:
            ...
            if counter % 5 == 4:
                ...
                # getting cacti in game
                if len(cacti.sprites()) < 1:
                    nearest_cactus = width - playerDino.rect.right
                else:
                    cacti_in_game = []
                    for i in cacti.sprites():
                        if i.rect.left > playerDino.rect.right:
                            cacti_in_game.append(i.rect.left)

                    if not cacti_in_game:
                        cacti_in_game = [width - playerDino.rect.right]

                    nearest_cactus = min(cacti_in_game)

                dino_action = nn_array[nn_playing].synapse([gamespeed, nearest_cactus])
                ...
                print(nn_array[nn_playing].fitness)
